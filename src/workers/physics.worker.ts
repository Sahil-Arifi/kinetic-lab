...
port.onmessage = (event) => {
  queue = queue.then(() => { runtime.handle(event.data); }).catch((failure: unknown) => {
    const parsed = commandSchema.safeParse(event.data);
    if (parsed.success) port.postMessage({
      protocolVersion: PROTOCOL_VERSION, sessionId: parsed.data.sessionId, sequence: 1,
      type: 'error', message: failure instanceof Error ? failure.message.slice(0, 500) : 'Physics initialization failed.',
    });
  });
};
