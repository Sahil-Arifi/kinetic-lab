...
export class FixedClock {
  ...
  reset(): void { this.previousMs = undefined; this.accumulator = 0; this.droppedTimeSeconds = 0; }
  clearWallTime(): void { this.previousMs = undefined; this.accumulator = 0; }
  advance(nowMs: number, step: () => void): number {
    if (!Number.isFinite(nowMs)) return 0;
    if (this.previousMs === undefined) { this.previousMs = nowMs; return 0; }
    ...
    while (this.accumulator + 1e-12 >= FIXED_DT && count < config.maxCatchUpSteps) {
      step(); this.accumulator -= FIXED_DT; count++;
    }
