...
export const bodySchema = z
  .strictObject({
    ...
    rotation: z.strictObject({
      x: finite.min(-Math.PI * 2).max(Math.PI * 2),
      y: finite.min(-Math.PI * 2).max(Math.PI * 2),
      z: finite.min(-Math.PI * 2).max(Math.PI * 2),
    }),
  ...
  .superRefine((body, ctx) => {
    if (body.bodyMode === 'dynamic' && body.mass <= 0)
      ctx.addIssue({
        code: 'custom',
        message: 'Dynamic bodies require positive mass',
        path: ['mass'],
      });
    if ((body.type === 'targetCup' || body.type === 'fixedBarrier') && body.bodyMode !== 'fixed')
      ctx.addIssue({
        code: 'custom',
        message: 'Cups and barriers must be fixed',
        path: ['bodyMode'],
      });
    if (
      body.type === 'sphere' &&
      (body.dimensions.x !== body.dimensions.y || body.dimensions.y !== body.dimensions.z)
    )
      ctx.addIssue({
        code: 'custom',
        message: 'Sphere dimensions must be equal',
        path: ['dimensions'],
      });
    if (
      body.type === 'targetCup' &&
      Math.min(body.dimensions.x, body.dimensions.y, body.dimensions.z) < 0.4
    )
      ctx.addIssue({
        code: 'custom',
        message: 'Cup dimensions must be at least 0.4',
        path: ['dimensions'],
      });
  });
    ...
    goals: z
      .array(
        z.strictObject({
          id: idSchema,
          bodyId: idSchema,
          sensorBodyId: idSchema,
          dwellTicks: z.number().int().min(1).max(1200),
        }),
      )
      .max(20),
  })
  .superRefine((scene, ctx) => {
    ...
    for (const goal of scene.goals) {
      if (goals.has(goal.id))
        ctx.addIssue({ code: 'custom', message: 'Duplicate goal ID', path: ['goals'] });
      goals.add(goal.id);
      if (bodies.get(goal.bodyId)?.bodyMode !== 'dynamic')
        ctx.addIssue({
          code: 'custom',
          message: 'Goal body must reference a dynamic body',
          path: ['goals'],
        });
      if (bodies.get(goal.sensorBodyId)?.type !== 'targetCup')
        ctx.addIssue({
          code: 'custom',
          message: 'Goal sensor must reference a target cup',
          path: ['goals'],
        });
    }
  });
...
export function parseScene(input: unknown): SceneDocument {
