...
import { CommandGate, PROTOCOL_VERSION, type ResponsePayload, type WorkerResponse } from './protocol';
...
export class PhysicsRuntime {
  ...
  constructor(private readonly output: (response: WorkerResponse) => void, private readonly now: () => number = () => performance.now()) {}

  private emit(payload: ResponsePayload): void {
    this.output({ ...payload, protocolVersion: PROTOCOL_VERSION, sessionId: this.sessionId, sequence: ++this.sequence });
  }
  ...
  handle(input: unknown): void {
    ...
    try {
      if (message.type === 'loadScene' || message.type === 'reset') {
        ...
        this.lastSnapshotMs = -Infinity; this.lastMetricsMs = -Infinity;
        ...
        switch (message.type) {
          case 'play': this.clock.clearWallTime(); this.playing = true; break;
          case 'pause': this.playing = false; this.clock.clearWallTime(); world.endGrab(); break;
          case 'step':
            if (this.playing) throw new Error('Pause before single stepping.');
            this.step(); break;
          case 'setGravity': world.setGravity(message.gravity); break;
          case 'beginGrab':
            if (!world.beginGrab(message.bodyId, message.target)) throw new Error('Only dynamic objects can be grabbed.');
            break;
          case 'moveGrab': world.moveGrab(message.target); break;
          case 'endGrab': case 'cancelGrab': world.endGrab(); break;
          case 'applyImpulse':
            if (!world.applyImpulse(message.bodyId, message.impulse)) throw new Error('Impulse target must be dynamic.');
            break;
        }
      }
      this.emit({ type: 'commandApplied', command: message.type, tick: this.world?.tick ?? 0, playing: this.playing });
      if (message.type !== 'moveGrab') this.publish(true);
    } catch (failure) {
      this.playing = false;
      this.world?.endGrab();
      this.clock.clearWallTime();
      this.emit({ type: 'error', message: failure instanceof Error ? failure.message.slice(0, 500) : 'Physics command failed.' });
    }
  }
  ...
  private publish(force: boolean, nowMs = this.now()): void {
    ...
    if (force || nowMs - this.lastMetricsMs >= 250) {
      this.emit({ type: 'metrics', tick: this.world.tick, stepDurationMs: this.stepDurationMs, ...this.world.metrics(), droppedTimeSeconds: this.clock.droppedTimeSeconds });
      this.lastMetricsMs = nowMs;
    }
  }

  dispose(): void {
    this.playing = false; this.world?.dispose(); this.world = undefined; this.clock.reset();
  }
}
