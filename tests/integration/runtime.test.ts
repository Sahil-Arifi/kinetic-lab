...
function harness() {
  ...
  let sequence = 0;
  const send = (payload: CommandPayload, sessionId = 'session-a') => runtime.handle({ ...payload, protocolVersion: 1, sessionId, sequence: ++sequence });
...
describe('worker runtime with real Rapier', () => {
  ...
  it('reconstructs on reset and rejects an old command against the new world', () => {
    ...
    expect(initial.tick).toBe(0); expect(initial.bodies[0]!.position).toEqual(minimalScene.bodies[0]!.position);
    const length = responses.length;
    send({ type: 'step' }); expect(responses).toHaveLength(length);
    expect(responses.filter(r => r.sessionId === 'session-b').map(r => r.sequence)).toEqual([1,2,3,4]);
    runtime.dispose();
  });
  it('applies gravity and constraint commands, reports invalid targets without crashing', () => {
    ...
    send({ type: 'step' }); send({ type: 'endGrab' }); send({ type: 'cancelGrab' });
    ...
    expect(responses.at(-1)?.type).toBe('error');
    send({ type: 'play' }); send({ type: 'step' });
    expect(responses.at(-1)?.type).toBe('error'); expect(runtime.playing).toBe(false);
    runtime.dispose();
  });
  it('publishes collision and goal events once in a sequenced session', () => {
    const { runtime, send, responses } = harness();
    send({ type: 'loadScene', scene: marbleRun }); send({ type: 'play' });
