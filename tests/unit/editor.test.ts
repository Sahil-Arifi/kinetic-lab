...
describe('authored scene operations', () => {
  it('edits every supported numeric field immutably and validates the result', () => {
    ...
    expect(edited.bodies[0]).toMatchObject({
      position: { x: 1, y: 2, z: 3 },
      mass: 2,
      friction: 0.6,
    });
  ...
  it.each(['sphere', 'block', 'domino', 'ramp', 'fixedBarrier', 'targetCup'] as const)(
    'adds a valid %s with deterministic collision-free IDs',
    (type) => {
      ...
      expect(second.bodies.at(-1)!.bodyMode).toBe(
        ['sphere', 'block', 'domino'].includes(type) ? 'dynamic' : 'fixed',
      );
    },
  );
...
describe('bounded authored history', () => {
  ...
  it('bounds past history and discards the abandoned redo branch after an edit', () => {
    ...
    for (const mass of [2, 3, 4])
      history = commitScene(history, editBody(history.present, 'marble', { mass }));
