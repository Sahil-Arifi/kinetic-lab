...
describe('main-thread client world isolation', () => {
  it('rejects old responses after reset, replacement and worker restart', () => {
    const ports: Port[] = [];
    const receive = vi.fn();
    const client = new PhysicsClient(receive, () => {
      const port = new Port();
      ports.push(port);
      return port;
    });
    ...
    const first = ports[0]!;
    const firstSession = first.messages[0]!.sessionId;
    ...
    first.respond({ ...response, sequence: 500 });
    expect(receive).toHaveBeenCalledTimes(1);
    const resetSession = first.messages.at(-1)!.sessionId;
    expect(resetSession).not.toBe(firstSession);
    first.respond({ ...response, sessionId: resetSession });
    expect(receive).toHaveBeenCalledTimes(2);
    client.load(minimalScene);
    first.respond({ ...response, sessionId: resetSession, sequence: 501 });
    expect(receive).toHaveBeenCalledTimes(2);
    ...
    expect(receive).toHaveBeenCalledTimes(2);
    const fresh = ports[1]!;
    fresh.respond({ ...response, sessionId: fresh.messages[0]!.sessionId });
  ...
  it('delivers every discrete command in sequence, validates before sending, and disposes once', () => {
    ...
    client.send({ type: 'endGrab' });
    client.send({ type: 'cancelGrab' });
    expect(port.messages.map((m) => m.sequence)).toEqual([1, 2, 3, 4, 5]);
    expect(port.messages.map((m) => m.type)).toEqual([
      'loadScene',
      'beginGrab',
      'moveGrab',
      'endGrab',
    ...
    expect(() =>
      client.load({ ...minimalScene, schemaVersion: 2 } as unknown as typeof minimalScene),
    ).toThrow();
    ...
    const onerror = port.onerror!;
    client.dispose();
    client.dispose();
    client.send({ type: 'play' });
    onmessage({ data: {} } as MessageEvent<unknown>);
    onerror({ message: 'late' } as ErrorEvent);
    expect(receive).toHaveBeenCalledTimes(2);
    expect(port.messages).toHaveLength(5);
