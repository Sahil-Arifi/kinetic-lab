...
import { CommandGate, ResponseGate, commandSchema, responseSchema } from '../../src/engine/protocol';
...
describe('scene and configuration contracts', () => {
  ...
  it.each([NaN, Infinity, -Infinity, -1, 0, 1001])('rejects invalid dynamic mass %s', mass => {
    const scene = structuredClone(minimalScene); scene.bodies[0]!.mass = mass;
    expect(() => parseScene(scene)).toThrow();
  });
  it.each([0, -1, 41, Infinity, NaN])('rejects unsafe dimensions %s', value => {
    const scene = structuredClone(minimalScene); scene.bodies[0]!.dimensions.x = value;
    expect(() => parseScene(scene)).toThrow();
  });
  it('rejects duplicate IDs, unsupported types, invalid versions and executable properties', () => {
    expect(() => parseScene({ ...minimalScene, bodies: [...minimalScene.bodies, ...minimalScene.bodies] })).toThrow();
    expect(() => parseScene({ ...minimalScene, bodies: [{ ...minimalScene.bodies[0], type: 'script' }] })).toThrow();
    for (const property of [{ schemaVersion: 2 }, { name: '<b>hello</b>' }, { name: 'https://evil.example' }, { callback: 'alert(1)' }, { script: () => 1 }]) expect(() => parseScene({ ...minimalScene, ...property })).toThrow();
  ...
  it('requires valid fixed cups and barriers, equal sphere dimensions, and goal references', () => {
    ...
    expect(() => parseScene({ ...minimalScene, bodies: [{ ...body, type: 'fixedBarrier' }] })).toThrow();
    expect(() => parseScene({ ...minimalScene, bodies: [{ ...body, type: 'targetCup', bodyMode: 'fixed', dimensions: { x: 0.2, y: 0.2, z: 0.2 } }] })).toThrow();
    expect(() => parseScene({ ...minimalScene, bodies: [{ ...body, dimensions: { x: 1, y: 0.5, z: 0.5 } }] })).toThrow();
    expect(() => parseScene({ ...minimalScene, goals: [{ id: 'goal', bodyId: 'absent', sensorBodyId: 'ball', dwellTicks: 1 }] })).toThrow();
    const goal = { id: 'goal', bodyId: 'ball', sensorBodyId: 'cup', dwellTicks: 24 };
    const valid = { ...minimalScene, bodies: [body, { ...body, id: 'cup', type: 'targetCup', bodyMode: 'fixed' }], goals: [goal] };
...
describe('protocol isolation', () => {
  ...
  it('validates each command and rejects malformed payloads', () => {
    ...
    const payloads = [
      { type: 'loadScene', scene: minimalScene }, { type: 'reset', scene: minimalScene },
      ...['play','pause','step','endGrab','cancelGrab'].map(type => ({ type })),
      { type: 'setGravity', gravity: target }, { type: 'beginGrab', bodyId: 'ball', target },
      { type: 'moveGrab', target }, { type: 'applyImpulse', bodyId: 'ball', impulse: target },
    ];
    for (const payload of payloads) expect(commandSchema.parse({ ...envelope, ...payload })).toEqual({ ...envelope, ...payload });
    for (const value of [null, 4, {}, { ...envelope, type: 'play', script: 'run' }, { ...envelope, type: 'unknown' }, { ...envelope, type: 'moveGrab', target: { ...target, x: NaN } }, { ...envelope, type: 'play', protocolVersion: 2 }]) expect(commandSchema.safeParse(value).success).toBe(false);
  });
  it('ignores old, duplicate, out-of-order, malformed and previous-session responses', () => {
    ...
    for (const type of ['ready','transforms','goalEvent','metrics','commandApplied','error']) expect(gate.accept({ ...ready, type, sequence: 999 })).toBeUndefined();
  ...
  it('only changes command sessions through load/reset and never revives retired worlds', () => {
    ...
    expect(gate.accept({ ...envelope, sessionId: 'session-c', sequence: 1, type: 'play' })).toBeUndefined();
  });
  it('validates every response variant', () => {
    const responses = [
      { type: 'ready' }, { type: 'commandApplied', command: 'play', tick: 1, playing: true },
      { type: 'transforms', tick: 1, bodies: [{ id: 'ball', position: { x: 0, y: 0, z: 0 }, rotation: { x: 0, y: 0, z: 0, w: 1 }, sleeping: false }] },
      { type: 'collisionEvent', bodyA: 'ball', bodyB: 'floor', started: true },
      { type: 'goalEvent', goalId: 'goal', bodyId: 'ball', tick: 4 },
      { type: 'metrics', tick: 1, stepDurationMs: 1, activeBodies: 1, sleepingBodies: 0, droppedTimeSeconds: 0 },
      { type: 'error', message: 'No body' },
    ];
    for (const response of responses) expect(responseSchema.safeParse({ ...envelope, ...response }).success).toBe(true);
  });
});
